...
/**
 * The workspace dashboard body. Shared by the live page and the design preview
...
export function WorkspaceOverview({
  ...
  // average of averages — a project with 12 milestones shouldn't weigh the
  // same as one with 2.
  ...
  const upcoming = active
    ...
    .filter((x): x is { project: ProjectWithMilestones; milestone: NonNullable<ReturnType<typeof nextMilestone>> } =>
      Boolean(x.milestone),
    )
    .sort((a, b) => {
    ...
    <div className="py-2 md:py-4">
      ...
      {projects.length === 0 ? (
        <section className="border-border bg-card mt-8 max-w-xl rounded-2xl border p-6 md:p-8">
          ...
          {addProjectSlot && <div className="mt-6 max-w-sm">{addProjectSlot}</div>}
        </section>
      ) : (
        <>
          <section
          ...
          <section aria-labelledby="overview-projects" className="mt-8">
            <div className="mb-3 flex items-baseline justify-between gap-4">
              ...
              <Link
                href={`/portal/${workspaceSlug}/projects`}
