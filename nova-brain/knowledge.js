...
//   1. nova-brain/prompts/*.js inject a compact summary into the system prompt
//   2. src/lib/strategist-tools.ts resolves lookup_site_info queries against this object
//
...
function getKnowledgeSummaryForPrompt() {
  const services = NOVA_KNOWLEDGE.services.map((s) => `- ${s.title}: ${s.summary}`).join("\n");
...
module.exports = { NOVA_KNOWLEDGE, getKnowledgeSummaryForPrompt };
